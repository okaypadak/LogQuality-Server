...
class DigerKayitlarManager:
    ...
    def create_diger_kayit(self, degisken, deger):
    ...
    def update_diger_kayit(self, kayit_id, new_degisken=None, new_deger=None):
        ...
        if diger_kayit:
            ...
            self.session.refresh(diger_kayit)
            return diger_kayit
        return None

    def delete_diger_kayit(self, kayit_id):
        diger_kayit = self.session.query(DigerKayitlar).filter_by(id=kayit_id).first()
        if diger_kayit:
            self.session.delete(diger_kayit)
            return True
        return False